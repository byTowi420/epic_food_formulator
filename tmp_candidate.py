...
class LabelTableDelegate(QStyledItemDelegate):
    """Custom grid painter with custom spans/indent and consistent borders."""
    ...
    def paint(self, painter: QPainter, option, index) -> None:  # type: ignore[override]
        ...
        opt.state &= ~QStyle.State_HasFocus  # avoid focus ring rounding the cell
        background_brush = opt.backgroundBrush
        has_manual_bg = (
            background_brush is not None
            and background_brush.style() != Qt.NoBrush
            and background_brush.color().alpha() > 0
        )
        is_selected = bool(opt.state & QStyle.State_Selected)
        # Always paint manual background so it survives selection state.
        if has_manual_bg:
            painter.save()
            painter.fillRect(opt.rect, background_brush)
            painter.restore()
            # Stop default highlight from replacing the manual color.
            if is_selected:
        ...
        # Subtle hover tint (also when selected) without hiding backgrounds.
        if opt.state & QStyle.State_MouseOver:
            hover_color = QColor(0, 0, 0, 24)
            painter.save()
        ...
        # Draw a simple outline for selection (manual bg stays visible).
        if is_selected:
            painter.save()
            border_color = QColor("#1a73e8") if (option.state & QStyle.State_Active) else QColor("#90caf9")
